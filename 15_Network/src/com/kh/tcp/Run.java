...
public class Run {
	 ...
	 * 
	 * - 서버에 요청하기위해서는 그 요청하고자하는 서버에 IP주소(또는 도메인), 포트번호를 알아야함
	 * 
	 * * 현재 구동중인 서버가 있을 때
	 *   클라이언트는 그 서버로 요청을 보낼 수 있고 => 응답결과가 돌아옴
	 *   
	 *   -요청과 응답에 의해서 웹은 작동합니다. ("통신한다")
	 *   -웹에서의 통신방식은 : HTTP프로토콜을 사용한다.
	 *   
	 ...
	 *   -UDP방식 : 데이터전송속도가 빠름! 신뢰성 없는 데이터 전달에 용이(데이터를 빠른속도로 전송하고자 할 때)
	 *   
	 *   
	 */
	...
	public static void main(String[] args) {
		...
		try {
			InetAddress localhost =InetAddress.getLocalHost();// 내PC(지역호스트)에 대한 정보를 변환

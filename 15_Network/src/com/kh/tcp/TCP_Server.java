...
public class TCP_Server {
	 ...
	 *	- 서버, 클라리언트간에 1:1 소켓통신
	 *	- 데이터를 교화하기에 앞서 서버, 클라이언트 연결이 되어야한다.(서버가 먼저 실행되어 클라이언트에 요청을 기달림)
	 *	- 신뢰성있는 데이터 전달가능
	 *
	 *	*Socket
	 *	- 프로세스간의 통신을 담당
	...
	public static void main(String[]args) {
		...
		ServerSocket server;
		try {
			// 2) ServerSocket객체생성시 포트 결합(Bind)
			...
			//PrintWriter : 데이터출력시 print(), println() 메소드 가지고 있는 보조스트림

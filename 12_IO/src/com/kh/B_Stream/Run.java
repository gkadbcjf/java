...
public class Run {
	/*
	 * 프로그램상의 데이터를 외부매체로 출력을 한ㄷ다거나 또는 외부매체로부터 입력을 받아오려고 한다.
	 * 이 때 반드시 외부매체와 연결되는 통로를 만들어줘야 한다! -> 스트림
	 * 
	 * * 스트림(통로)의 특징
	 * - 단반향 : 입력이면 입력 / 출력이면 출력만 가능
	 *          => 입력과 출력을 동시에 하고자 한다면 => 입력용스트럼.출력용스트럼 따로 열어야 한다.
	 *          
	 * - 선입선출(FIFO) : 통로가 파이프같은 개념이기 때문에 먼저 보낸값이 먼저 나간다.

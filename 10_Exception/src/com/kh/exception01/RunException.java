...
//import java.io.BufferedReader;  컨트롤 쉬프트 o
...
//		System.out.println("나누기 결과 : " +result);
		...
		/*
		 * "예외"가 발생했을 경우에 대해서 "처리"하는 방법을 "예외처리"
		 * try {
		 * 		예외가 발생할 수 있는 구문;
	...
	public void method02() {
		...
		// NegativeArraySizeException : 배열의 할당 과정에서 음수로 지정하면 발생되는 예외
		...
		System.out.println("100번째 인덱스 값 : " + arr[100]);
			...
			System.out.println("배열의 크기를 음수 지정할 수 없습니다. ");
		}
	...
	public void method03() throws IOException  {
		/*
		 * CheckedException : 반두사 예외처리를 해야되는 예외들
		 * => 조건물을 미리 제시할 수 없음(왜? 에측이 불가한 곳에서 문제가 발생하디 때문에 예외처리가 필수이다.)
		 * => 외부 매개체와 입출력이 일어날 때 발생된다.
		 *
		 */
		//Scanner와 같이 키보드로 값을 입력받을 수 있는 객체(단, 문자열로 읽오들여인다.)
		...
		// 2. throws : 지금 이자리에서 예외를 처리하지않고 현재 이 매소드를 호출한 곳으로 예외처리를 떠넘기겠다.
		String str = br.readLine();
	}

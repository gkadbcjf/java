...
public class Run {
	 ...
	 *  - 컴파일에러 : 소스코드 문법사이의 오류 => 빨간줄로 애초에 오류로 알려준다. ( 개발자의 실수 ) => 발견과 해결이 쉽다.
	 *  - 런타임에러 : 코드 상으로 문제가 없는데 프로그램 실행 중에 발생하는 에러( 사용자의 실수일 수 있고 개발자가 예측 가능하지만 처리를 안해놓은 실수 )
	 *  - 논리에러  : 문법적으로도 문제없고 실행했을 때도 문제는 없지만 프로그램의도와 맞지않는 동작이나 결과가 나타나는 에러
	 *  
	 *  컴파일에러, 런타임에러, 논리에러같은 개발자가 예측하여 수정할 수 있는 에러들을 가지고 작업
...
//				ex.method04();
//			} catch(IOException e) {
//				e.printStackTrace(); //에러가 어디서 시작해서 어디까지 왔는지 출력해주는 것
//				}

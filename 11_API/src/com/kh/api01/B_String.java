...
public class B_String {
	...
	public void method01() {
		...
		// String클래스에 toString메소드 이미 오버라이딩 되어있음!(실제 담긴 문자열을 반환 하도록)

		System.out.println(str1 == str2); // false (주소 값 비교라서)

		System.out.println(str1.equals(str2));
		// String클래스에 equals메소드는 이미 오버라이딩 되어있음(주소값비교가 아닌 담긴 문자열을 비교하도록)

		System.out.println(str1.hashCode());
		System.out.println(str2.hashCode()); // 문자열 해시코드
		// String클래스에 hashCode매소드 이미 오버라이딩 되어있다. (주소값이 아닌 문자열을 가지고 만들도록)

		// 주소값에 대한 해시코드가 알고 싶다면 identityHashCode매소드를 이용해야한다.
		System.out.println(System.identityHashCode(str1));
		System.out.println(System.identityHashCode(str2)); // 주소값 해시코드

		System.out.println("---------------------------------------------");

		
		// 2. 문자열을 리터럴값으로 생성
		String str3 = "hello"; // Heap영역에 String pool(상수풀)에 주소값을 저장
		String str4 = "hello";
		...
		str4 = "hi";
		System.out.println("str4 = hi일시 : " + System.identityHashCode(str4));

		// String클래스 == 불변클래스(변하지않는 클래스)
		// 변경이 가능하지만 수정되는 개념이 아니라 새로 생성하여 교체하는 개념

		str3 = "bye";
		System.out.println("str3 = bye일시 : " + System.identityHashCode(str3));
	...
	public void method02() {
		...
		// 2. 문자열.comcat(String str) : String
		// 문자열과 전달된 또다른 문자열을 하나로 합쳐서 새로운 문자열로 리턴
		String str2 = str1.concat("!!!"); // 객체를 생성(새로운 주소값만듬)
		System.out.println("str2 : " + str2);

		String str3 = str1 + "!!!";// 상수처리 (상수풀에 새로 생김)
		...
		// 문자열과 전달된 또 다른 문자열을 가지고 주소값 비교값 아닌 실제 문자열 값을 가지고 동등비교
		...
		// 6. 문자열.substring(int beginIndex) : String => 문자열의 beginINDEX위치에서부터 끝까지 문자열
		// 추출해서 리턴
		...
		// 7. 문자열.replace(cahr oldChar, char newChar) : String
		// 문자열에서의 oldChar문자열을 newChar문자로 변환한 새 문자열 리턴
		String str4 = str1.replace('l', 'c'); //replace("hello","jc");도 가능
		System.out.println("str4 : " + str4);
		System.out.println("str1 : " + str1);
		String hay = "hello cheeze";
		hay = hay.replaceAll("hel", "ffff");
		System.out.println(hay);
		
		// 8. 문자열.toUpperCase() : String => 문자열을 다 대문자로 변경한 새 문자열로 리턴한다.
		// 문자열.toLowerCase() : String => 문자열을 다 소문자로 변경한 새 문자열로 리턴한다.
		 ...
		 *  System.out.println(i + "번 째 인덱스 : " +
		 * arr[i]); }
		 */
		int i = 0;
		for (char ch1 : arr) {

			System.out.println(i++ + "번 째 인덱스 : " + ch1);
		}
	...
	public void method03() {
		...
		//방법2. 분리된 문자열들을 각각 토큰으로써 관리하고자 할 떄
		...
		//방법 2.
		while(stn.hasMoreTokens()) {  //hasMoretTokens는 StringTokenizer 객체가 토큰이 있는지 없는지 확인
			System.out.println(stn.nextToken());
		}

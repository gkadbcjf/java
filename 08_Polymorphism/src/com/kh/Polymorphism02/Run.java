...
//		System.out.println("3. 부모타입 래퍼런스로 자식객체를 다루는 경우(다형성)");
//		Parent p2 = /*(Parent)*/new Child1(); //자동형변환 됨(업캐스팅)
//		p2.printParnet();
//		((Child1)p2).printParnet(); //강제형변환을 하면 호출 가능!!(다운캐스팅)
//		//p2p 래퍼런스로 지금당장은 Parent에만 접근이 가능하지만
...
//		((Child2)arr[3]).printChild2();
		 ...
		 * 현재 래퍼런스가 실질적으로 어떤 클래스타입을 참조하는지 확인 할 때 사용
		 */
		
		
		for(int i=0; i<arr.length; i++) {
			if(arr[i] instanceof Child1) { // 해당 참조변수가 실제 Child1을 참조하고 있나?(arr[i]가 Child1공간 사용가능해?)
